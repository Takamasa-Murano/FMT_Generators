...
////////////Prependの設定　デフォルトから条件に応じて置換・追加////////////////////////////////////////////////////
          let pre_text = "";
          let pre_pcOV= (function(){/*<div id="geniee_overlay" style="position: fixed; bottom: 0px; height: 60px; left: 0; right: 0; z-index: 100; ">
...
////////////Appendの設定　デフォルトから条件に応じて追加　オプション並べて条件ごとに使用判断////////////////////////////
          //デフォルト
          let app_default = (function(){/*<div id="gnov_cancel_area">
<script>
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let skip_in_ydn = (function(){/*if(typeof window.__gn_ov_yads_params != "undefined") return;
*/}).toString().match(/\/\*([^]*)\*\//)[1];
...
(function(d) {
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_zoom_post = (function(){/*var s = window.innerWidth / */}).toString().match(/\/\*([^]*)\*\//)[1] + BANNER_WIDTH + (function(){/*,
BOOTS_HEIGHT = */}).toString().match(/\/\*([^]*)\*\//)[1] + PR_HEIGHT + (function(){/*;
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let app_zoom_pc_close =  (function(){/*<script>
(function(window, document){
...
(function(d) {
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_pr_post = (function(){/*var AD_STR = "*/}).toString().match(/\/\*([^]*)\*\//)[1] + PR_TEXT + (function(){/*",
BOTTOM_HEIGHT = "*/}).toString().match(/\/\*([^]*)\*\//)[1] + PR_HEIGHT + (function(){/*px",
...
div.setAttribute("style", "background-color:*/}).toString().match(/\/\*([^]*)\*\//)[1] + PR_BACKGROUND + (function(){/*;text-align:center;width:*/}).toString().match(/\/\*([^]*)\*\//)[1] + BANNER_WIDTH + (function(){/*px;color:*/}).toString().match(/\/\*([^]*)\*\//)[1] + PR_FONTCOLOR + (function(){/* !important;margin: 0 auto; font-size:*/}).toString().match(/\/\*([^]*)\*\//)[1] + PR_FONTSIZE + (function(){/*px;");
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          //ページ下部での上部移行
          let app_up_shift1_pre = (function(){/*<script type="text/javascript"> 
(function(global) { 
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_up_shift1_post = (function(){/*var d = global.document, 
w = global.window, 
h = 0, 
banner_height = */}).toString().match(/\/\*([^]*)\*\//)[1] + BANNER_HEIGHT + (function(){/*,
boots_height = */}).toString().match(/\/\*([^]*)\*\//)[1] + PR_HEIGHT + (function(){/*,*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_up_shift_zoom1 = (function(){/*
s = window.innerWidth / */}).toString().match(/\/\*([^]*)\*\//)[1] + BANNER_WIDTH + (function(){/*,*/}).toString().match(/\/\*([^]*)\*\//)[1];
...
ov_base.style.bottom = boots_height*/}).toString().match(/\/\*([^]*)\*\//)[1]; 
          let app_up_shift_zoom2 = (function(){/* * s*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_up_shift3 = (function(){/* + "px";
ov_base.style.top = "";
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let app_under_space_pre = (function(){/*<script> 
(function(window, document) { 
window.addEventListener('DOMContentLoaded', function() { 
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_under_space_post1 = (function(){/*var a = document.getElementById("geniee_overlay");
...
d.style.width = '100%';
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_under_space_post_zoom = (function(){/*var s =  window.innerWidth / BANNER_WIDTH;
t = parseFloat(a.style.height) * s;
if(b) t += parseFloat(b.style.height) * s;
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_under_space_post2 = (function(){/*d.style.height = t + 'px';
...
<script> 
  (function(window, document) { 
    ...
    var COLOR = "*/}).toString().match(/\/\*([^]*)\*\//)[1] + bg_color + (function(){/*";
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let inst_hidden = (function(){/*<script>
  (function(window, document){
    window.addEventListener("load", function(){
      var a = document.getElementById("geniee_overlay");
      var b = document.getElementById("*/}).toString().match(/\/\*([^]*)\*\//)[1] + INST_AREA_ID + (function(){/*");
      var c = document.getElementById("*/}).toString().match(/\/\*([^]*)\*\//)[1] + INST_CLOSEBTN_ID + (function(){/*");
      if(c){
          a.style.opacity = "0";
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let pre_scroll_hidden = (function(){/*<script type="text/javascript"> 
(function(window, document) { 
var a = document.getElementById("geniee_overlay");
window.addEventListener("load", function() { 
a.style.opacity = 0;
...
if(sc_top > */}).toString().match(/\/\*([^]*)\*\//)[1] + PRE_SCROLL_HEIGHT + (function(){/* && a.style.opacity == "0") a.style.opacity = 1;
}, false); 
}, false); 
})(window, document);
</script>
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let app_forPC = (function(){/*</div>
</div>
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          let app_forPC_isCloseBtn = (function(){/*<script>
(function(window, document) {
    ...
    a.onclick = function() {
        ...
        b.style.height = "0px";
          ...
          //ダブルオーバーレイ時の追加記述
          let app_double_setting = (function(){/*<script>
(function(window, document) {
    var a = document.getElementById('geniee_overlay').getElementsByTagName('div')[0];
    a.style.width = '941px';
    a.style.margin = 'auto';
...
////////////アドソース-タグ欄の設定////////////////////////////////////////////////////////////////////////////
          ...
          //YDNの場合
          let ad_tag_ydn = (function(){/*<!-- YDNアドソースのタグ欄にのみ記入する箇所　ここから -->
<script type="text/javascript">
window.__gn_ov_yads_params = true;
</script>
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let ad_tag_height = (function(){/*<script>
(function(window,document){
  var BANNER_WIDTH = 320;
  var a = document.getElementById("geniee_overlay");
  if(!a) a = parent.document.getElementById("geniee_overlay");
  var b = a.getElementsByTagName("iframe")[0];
  a.style.height = "*/}).toString().match(/\/\*([^]*)\*\//)[1] + AD_SOURCE_INDIVIDUAL_HEIGHT + (function(){/*px";
  if(b){
    b.height = */}).toString().match(/\/\*([^]*)\*\//)[1] + AD_SOURCE_INDIVIDUAL_HEIGHT + (function(){/*;
    b.style.height = "*/}).toString().match(/\/\*([^]*)\*\//)[1] + AD_SOURCE_INDIVIDUAL_HEIGHT + (function(){/*px";
  }
})(window,document);
</script>
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          //PRバー削除
          let ad_tag_prbar_delete = (function(){/*<script>
  (function(window, document){
    window.addEventListener('load', function(){
...
*/}).toString().match(/\/\*([^]*)\*\//)[1];
          ...
          let ad_tag_double_setting = (function(){/*<div style="float:left; clear:none; width:468px; height:60px; text-align:center; margin-right:5px;">
